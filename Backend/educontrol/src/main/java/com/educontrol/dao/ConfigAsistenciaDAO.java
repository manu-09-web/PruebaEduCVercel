...
public class ConfigAsistenciaDAO {
    ...
    public void crear(ConfigAsistencia config) throws SQLException {
        String sql = "INSERT INTO config_asistencia (porcentaje, idCampoFormativo, idUsuario) VALUES (?, ?, ?)";
    ...
    public void actualizar(ConfigAsistencia config) throws SQLException {
        String sql = "UPDATE config_asistencia SET porcentaje = ?, idCampoFormativo = ?, idUsuario = ? WHERE idConfigAsistencia = ?";
        ...
        try (Connection conn = Main.conectar();
            ...
            stmt.setInt(2, config.getIdCampoFormativo());
            stmt.setInt(3, config.getIdUsuario());
            stmt.setInt(4, config.getIdConfigAsistencia());
