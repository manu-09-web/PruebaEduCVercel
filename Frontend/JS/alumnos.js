...
// --- Nombres de grupo: se piden UNA VEZ a la BD (GET /grupos) y se cachean ---
let gruposCache = []; // [{ idGrupo, grado, grupo }]
...
// Llena un <select> con las opciones reales de grupo, seleccionando idGrupoActual si se indica
...
// --- Sesión: usada en varias vistas para saber el rol ---
...
document.addEventListener('DOMContentLoaded', async () => {
    // Sidebar activo (todas las páginas)
    const items = document.querySelectorAll('.item-menu');
    items.forEach(item => {
        if (item.innerText.trim() === "Alumno") item.classList.add('active');
    });

    // Cargamos los grupos reales de la BD ANTES de renderizar cualquier cosa que los use
    await cargarGruposDesdeAPI();

    // Detecta en qué página estamos y ejecuta la lógica correspondiente
...
async function inicializarRegistrarAlumno() {
    ...
    // Si es Docente, ocultamos el campo de grupo (el backend lo fuerza a su propio grupo de todas formas)
    // Si es Director, convertimos el input numérico en un <select> con los grupos reales de la BD
    ...
    formRegistrar.addEventListener('submit', async (e) => {
        ...
        // Solo se manda idGrupo si es Director (el campo existe y es visible)
