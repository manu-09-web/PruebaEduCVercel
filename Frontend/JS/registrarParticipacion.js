    ...
    try {
        ...
        alumnos.forEach((alumno, index) => {
            ...
            fila.innerHTML = `
                ...
                <td><input type="number" class="input-tabla-pildora input-puntuacion" placeholder="0" min="5" max="10" step="0.1"></td>
            `;
...
async function guardarRegistro() {
    ...
    for (const fila of filas) {
        ...
        const puntuacionInput = fila.querySelector('.input-puntuacion').value;
        const puntuacion = parseFloat(puntuacionInput);

        if (isNaN(puntuacion)) {
            continue; // Alumno sin puntuacion capturada, se omite
        }
        ...
        try {
            const response = await fetch(`${API_URL}/registro-participacion`, {
                method: 'POST',
    ...
    if (huboFueraDeRango) {
        mostrarError('La puntuacion debe estar entre 5 y 10. Revisa los alumnos marcados fuera de rango.');
        return;
    }
