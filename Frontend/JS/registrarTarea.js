    ...
    try {
        ...
        alumnos.forEach((alumno, index) => {
            ...
            fila.innerHTML = `
                ...
                <td><input type="radio" name="tarea_alumno_${alumno.matricula}" value="Entrego" class="radio-asistencia radio-verde" checked></td>
                <td><input type="radio" name="tarea_alumno_${alumno.matricula}" value="Incompleta" class="radio-asistencia radio-amarillo"></td>
                <td><input type="radio" name="tarea_alumno_${alumno.matricula}" value="No entrego" class="radio-asistencia radio-rojo"></td>
...
async function guardarRegistro() {
    ...
    const nombreTarea = document.getElementById('input-tarea-nombre').value.trim();
    ...
    if (!nombreTarea) {
        mostrarError('Escribe el nombre de la tarea');
        return;
    }
    ...
    for (const fila of filas) {
        ...
        const radioSeleccionado = fila.querySelector(`input[name="tarea_alumno_${matricula}"]:checked`);
        const estatus = radioSeleccionado ? radioSeleccionado.value : 'Entrego';
        const observaciones = fila.querySelector('.input-observaciones').value.trim();

        try {
            const response = await fetch(`${API_URL}/registro-tarea`, {
                method: 'POST',
